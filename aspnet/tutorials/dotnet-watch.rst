...
So far we only used ``dotnet watch`` to run our application. The file watcher can also be used to run other ``dotnet`` commands like ``test`` or ``publish``.
...
In either case, once all the tests run, the watcher will indicate that it waits for a file to change before restarting ``dotnet test``.
